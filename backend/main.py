...
class GenerateFromDescriptionRequest(AudioGenrationBase):
    full_described_song:str   

class GenerateWithCustomLyricsRequest(AudioGenrationBase):
    prompt:str         
    lyrics:str

class GenerateWithDescribedLyricsRequest(AudioGenrationBase):
    prompt:str         
...
class MusicGenServer:
    ...
    def load_model(self):
        ...
        # Stable Diffusion Model(thumbnail generation)
    ...
    def prompt_qwen(self, question: str):
        #Function to prompt the Qwen model with a question and return the response
        messages = [
    ...
    def generate_prompt(self, description:str):
        #Insert description into template and Run LLM inference and return the generated prompt
    ...
    def generate_and_upload_to_s3(
        ...
        thumbnail_prompt = f"Generate a cover image for the song with the following description: {prompt}"
    ...
    def gen(self) -> GenerateMusicResponse:
        output_dir ="/temp/output"
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir,f"{uuid.uuid4()}.wav")
    ...
    def generate_from_description(self, request:GenerateFromDescriptionRequest) -> GenerateMusicResponseS3:
        #Generating a prompt and Lyrics from the description
...
def main():
    ...
    # Change this to use the correct request model
    request_data = GenerateWithDescribedLyricsRequest(
        prompt="rave,funk, 140BPM, disco",  # Changed from full_described_song
        described_lyrics="Lyrics about water bottles",  # Add this field
        guidance_scale=15
    )
