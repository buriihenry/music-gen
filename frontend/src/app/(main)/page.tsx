...
import { SongCard } from '~/components/home/home-card';
...
export default async function Page() {
  ...
  const categorizedSongs = songsWithUrls.filter((song) =>!trendingSongsIds.has(song.id) 
  && song.categories.length > 0,).reduce((acc, song) =>{
  ...
  if (trendingSongs.length ===0 && Object.keys(categorizedSongs).length===0){
    return <div className='flex h-full flex-col items-center justify-center p-4 text-center'>
      ...
      <p className='text-muted-foreground mt-2'>
        There are no Published songs available now. Check back later!
      </p>
    </div>
